...
concept is_optional_runner =
    std::same_as<std::nullopt_t, OPT_RUNNER> || requires {
      requires is_runner<typename OPT_RUNNER::value_type>;
      requires std::same_as<OPT_RUNNER,
                            std::optional<typename OPT_RUNNER::value_type>>;
    };
...
concept is_builder =
    is_runner<BUILDER> && std::derived_from<builder_base, BUILDER> &&
    requires(const fs::path path) {
...
public:
  ...
  static constexpr basic_builder::factory create =
      [](work_dir dir, env::environment::optional env) {
        if (!is_root(dir)) {
