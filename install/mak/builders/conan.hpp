...
struct conan: basic_builder<conan_spec, conan> {
    ...
    static inline constexpr auto profiles = std::array {
        conan_profile{"gcc:build/gcc/generators"sv},
        conan_profile{"clang:build_clang/clang/generators"sv}
    };

    constexpr static auto find_profile(std::string_view profile) 
    {
        if (auto build_dir_it = std::ranges::find(profiles, profile, &conan_profile::name); build_dir_it != std::end(profiles)) {
            return *build_dir_it;
        } else {
